...
class Keuangan extends CI_Controller {
  ...
  public function index(){
    ...
    public function nuwas(){
  	...
  	return view('pages.keuangan_nuwas');
  }
    ...
    $query="select p.id as key,p.nama as key_name,
     ...
     CASE WHEN (sum(t.total_anggaran) is NULL) THEN 100 ELSE SUM(a.anggaran)*100 /sum(t.total_anggaran)  END  as data_Persentase_anggaran_terhadap_total_daerah
      from view_daerah as p
       ...
       {{where}}
      group by p.id ,p.nama order by sum(a.anggaran) desc
    ";
   ...
   public function per_program(){
    ...
    $query="
     ...
     sum(a.anggaran) as data_Total_Anggaran_kegiatan
      ...
      {{where}} 
      group by (a.kode_program),a.uraian_kode_program_daerah  order by sum(a.anggaran) desc
